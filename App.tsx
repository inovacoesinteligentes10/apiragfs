        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                const doc: ProcessedDocument = {
                    ...
                    size: file.size,
                    textLength: 0, // Will be updated after processing
                    extractionMethod: 'Gemini File API',
                    chunks: 0, // Will be estimated
                    processingTime: 0,
                    status: 'processing',
                    uploadDate: new Date()
                };
                ...
                try {
                    ...
                    const estimatedTextLength = Math.floor(file.size * 0.8); // Rough estimate
                    const estimatedChunks = Math.ceil(estimatedTextLength / 1000);
            ...
            setCurrentView('chat'); // Navigate to chat
    ...
    return (
        <main className="h-screen flex bg-slate-900">
            <Sidebar
                ...
                hasActiveSession={status === AppStatus.Chatting}
            />
            {renderMainContent()}
        </main>
    );
